...
def run_tests_from_file(filename):
    try:
        interpreter = Interpreter()  # Persistent instance
                ...
                if line and not line.startswith("#"):
                    try:
                        ...
                        for statement in statements:
                            result = interpreter.evaluate(statement)  # Use persistent interpreter
...
def run_interactive_mode():
    ...
    while True:
        try:
            text = input("calc > ").strip()  # Add "calc > " prompt
            ...
            statements = parser.parse()
            for statement in statements:
                interpreter.evaluate(statement)  # Use the same interpreter instance
        except Exception as e:
            print("Error:", e)
