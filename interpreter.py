...
class Interpreter:
    ...
    def visit_list_assign(self, node):
        ...
        if isinstance(node.list_expr, IdentifierNode):
            self.scopes[-1][node.list_expr.identifier] = lst
        return lst
    ...
    def visit_add(self, node):
        ...
        if isinstance(left, String) or isinstance(right, String):
            left_str = str(left.value) if not isinstance(left, String) else left.value
            right_str = str(right.value) if not isinstance(right, String) else right.value
            # Remove .0 from whole numbers in strings
    ...
    def visit_power(self, node):
        ...
        if isinstance(left, Number) and isinstance(right, Number):
            return Number(left.value ** right.value)
        raise TypeError("Unsupported operands for exponentiation")
    ...
    def visit_index(self, node):
        ...
        if isinstance(container, List):
            if not isinstance(index, Number):
                raise TypeError(f"List index must be a number, got {type(index).__name__}")
            idx = int(index.value)
            if idx < 0 or idx >= len(container.elements):
                raise IndexError(f"List index {idx} out of range (length: {len(container.elements)})")
            return container.elements[idx]
        elif isinstance(container, Dict):
            return self.visit_dict_access(DictAccessNode(node.list_identifier, node.index_expr))
        else:
            raise TypeError(f"Expected list or dictionary, got {type(container).__name__}")

    def visit_dict_access(self, node):
        dict_obj = self.evaluate(node.dict_expr)
        if not isinstance(dict_obj, Dict):
            if isinstance(dict_obj, List):
                return self.visit_index(IndexNode(node.dict_expr, node.key_expr))
            raise TypeError(f"Expected dictionary or list, got {type(dict_obj).__name__}")
        if not dict_obj.elements:
            raise KeyError("Cannot access key in empty dictionary")
        key = self.evaluate(node.key_expr)
        if isinstance(key, String):
            key = key.value
        elif isinstance(key, (Number, Boolean)):
            key = key.value
        else:
