...
class Interpreter:
    def __init__(self):
        self.globals = {}  # Dictionary to store global variables
        self.visit_map = {
    ...
    def visit_add(self, node):
        ...
        if isinstance(left, String) and isinstance(right, String):
            # Concatenate strings without creating intermediate copies
            return String(left.value + right.value)

        if isinstance(left, Number) and isinstance(right, Number):
            return Number(left.value + right.value)

        raise TypeError(f"Cannot add a {type(left).__name__} and a {type(right).__name__}")
    ...
    def visit_equals(self, node):
        ...
        if type(left) != type(right):
            return Boolean(False)  # Different types are not equal

        if isinstance(left, String) and isinstance(right, String):
            return Boolean(left.value == right.value)  # Compare strings

        if isinstance(left, Number) and isinstance(right, Number):
            return Boolean(left.value == right.value)  # Compare numbers

        if isinstance(left, Boolean) and isinstance(right, Boolean):
            return Boolean(left.value == right.value)  # Compare booleans

        raise TypeError(f"Cannot compare a {type(left).__name__} and a {type(right).__name__}")
    ...
    def visit_less_than(self, node):
        ...
        if isinstance(left, Number) and isinstance(right, Number):
            return Boolean(left.value < right.value)  # Fix: Properly compare numbers
        if isinstance(left, String) and isinstance(right, String):
            return Boolean(left.value < right.value)
